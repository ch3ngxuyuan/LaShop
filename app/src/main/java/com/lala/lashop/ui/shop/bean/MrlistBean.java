package com.lala.lashop.ui.shop.bean;

import android.os.Parcel;
import android.os.Parcelable;

/**
 * Created by JX on 2018/4/13.
 */

public class MrlistBean implements Parcelable {

    /**
     * a_id :
     * aadm :
     * aadmname :
     * actid :
     * adate :
     * addr :
     * admin_id : 10030
     * c_id :
     * clds : []
     * color : 卡其色
     * comment_body :
     * distribution :
     * dmark :
     * enddate :
     * fa_date :
     * groupid : 20140808080808
     * gui : 均码
     * haveping :
     * id : 281
     * isFanli : 0
     * islimit :
     * isorder :
     * isscore :
     * jmark :
     * lang_id :
     * limit :
     * m_date :
     * name :
     * nowcount : 993
     * openid :
     * or_date : 2018-04-12 22:53:05
     * or_flag : 0
     * order_id : 180412225305872
     * payStatus : 10042
     * pay_pwd :
     * payway :
     * phone :
     * prepay_id :
     * profit :
     * sa : null
     * selected : true
     * shou_date :
     * smark :
     * sp : null
     * sp_count : 1
     * sp_dealer :
     * sp_grounding :
     * sp_id : 1597
     * sp_postage : 0
     * sp_price : 180.00
     * sp_simg : /upload/20170210/170240902.jpg
     * sp_title : Diller情侣杯
     * sp_total : 180.00
     * tmark :
     * twuliu :
     * twuliuid :
     * u_account : 15777777777
     * u_delete : 0
     * u_id :
     * u_phone :
     * uadm :
     * uadmname :
     * udate :
     * vir :
     * virid :
     * wuliu :
     * wuliuid :
     * wxpay :
     * youfei :
     * youmo :
     * yunfei : 0.00
     */

    private String a_id;
    private String aadm;
    private String aadmname;
    private String actid;
    private String adate;
    private String addr;
    private String admin_id;
    private String c_id;
    private String color;
    private String comment_body;
    private String distribution;
    private String dmark;
    private String enddate;
    private String fa_date;
    private String groupid;
    private String gui;
    private String haveping;
    private String id;
    private int isFanli;
    private String islimit;
    private String isorder;
    private String isscore;
    private String jmark;
    private String lang_id;
    private String limit;
    private String m_date;
    private String name;
    private String nowcount;
    private String openid;
    private String or_date;
    private String or_flag;
    private String order_id;
    private String payStatus;
    private String pay_pwd;
    private String payway;
    private String phone;
    private String prepay_id;
    private String profit;
    private boolean selected;
    private String shou_date;
    private String smark;
    private String sp_count;
    private String sp_dealer;
    private String sp_grounding;
    private String sp_id;
    private String sp_postage;
    private String sp_price;
    private String sp_simg;
    private String sp_title;
    private String sp_total;
    private String tmark;
    private String twuliu;
    private String twuliuid;
    private String u_account;
    private String u_delete;
    private String u_id;
    private String u_phone;
    private String uadm;
    private String uadmname;
    private String udate;
    private String vir;
    private String virid;
    private String wuliu;
    private String wuliuid;
    private String wxpay;
    private String youfei;
    private String youmo;
    private String yunfei;

    public String getA_id() {
        return a_id;
    }

    public void setA_id(String a_id) {
        this.a_id = a_id;
    }

    public String getAadm() {
        return aadm;
    }

    public void setAadm(String aadm) {
        this.aadm = aadm;
    }

    public String getAadmname() {
        return aadmname;
    }

    public void setAadmname(String aadmname) {
        this.aadmname = aadmname;
    }

    public String getActid() {
        return actid;
    }

    public void setActid(String actid) {
        this.actid = actid;
    }

    public String getAdate() {
        return adate;
    }

    public void setAdate(String adate) {
        this.adate = adate;
    }

    public String getAddr() {
        return addr;
    }

    public void setAddr(String addr) {
        this.addr = addr;
    }

    public String getAdmin_id() {
        return admin_id;
    }

    public void setAdmin_id(String admin_id) {
        this.admin_id = admin_id;
    }

    public String getC_id() {
        return c_id;
    }

    public void setC_id(String c_id) {
        this.c_id = c_id;
    }

    public String getColor() {
        return color;
    }

    public void setColor(String color) {
        this.color = color;
    }

    public String getComment_body() {
        return comment_body;
    }

    public void setComment_body(String comment_body) {
        this.comment_body = comment_body;
    }

    public String getDistribution() {
        return distribution;
    }

    public void setDistribution(String distribution) {
        this.distribution = distribution;
    }

    public String getDmark() {
        return dmark;
    }

    public void setDmark(String dmark) {
        this.dmark = dmark;
    }

    public String getEnddate() {
        return enddate;
    }

    public void setEnddate(String enddate) {
        this.enddate = enddate;
    }

    public String getFa_date() {
        return fa_date;
    }

    public void setFa_date(String fa_date) {
        this.fa_date = fa_date;
    }

    public String getGroupid() {
        return groupid;
    }

    public void setGroupid(String groupid) {
        this.groupid = groupid;
    }

    public String getGui() {
        return gui;
    }

    public void setGui(String gui) {
        this.gui = gui;
    }

    public String getHaveping() {
        return haveping;
    }

    public void setHaveping(String haveping) {
        this.haveping = haveping;
    }

    public String getId() {
        return id;
    }

    public void setId(String id) {
        this.id = id;
    }

    public int getIsFanli() {
        return isFanli;
    }

    public void setIsFanli(int isFanli) {
        this.isFanli = isFanli;
    }

    public String getIslimit() {
        return islimit;
    }

    public void setIslimit(String islimit) {
        this.islimit = islimit;
    }

    public String getIsorder() {
        return isorder;
    }

    public void setIsorder(String isorder) {
        this.isorder = isorder;
    }

    public String getIsscore() {
        return isscore;
    }

    public void setIsscore(String isscore) {
        this.isscore = isscore;
    }

    public String getJmark() {
        return jmark;
    }

    public void setJmark(String jmark) {
        this.jmark = jmark;
    }

    public String getLang_id() {
        return lang_id;
    }

    public void setLang_id(String lang_id) {
        this.lang_id = lang_id;
    }

    public String getLimit() {
        return limit;
    }

    public void setLimit(String limit) {
        this.limit = limit;
    }

    public String getM_date() {
        return m_date;
    }

    public void setM_date(String m_date) {
        this.m_date = m_date;
    }

    public String getName() {
        return name;
    }

    public void setName(String name) {
        this.name = name;
    }

    public String getNowcount() {
        return nowcount;
    }

    public void setNowcount(String nowcount) {
        this.nowcount = nowcount;
    }

    public String getOpenid() {
        return openid;
    }

    public void setOpenid(String openid) {
        this.openid = openid;
    }

    public String getOr_date() {
        return or_date;
    }

    public void setOr_date(String or_date) {
        this.or_date = or_date;
    }

    public String getOr_flag() {
        return or_flag;
    }

    public void setOr_flag(String or_flag) {
        this.or_flag = or_flag;
    }

    public String getOrder_id() {
        return order_id;
    }

    public void setOrder_id(String order_id) {
        this.order_id = order_id;
    }

    public String getPayStatus() {
        return payStatus;
    }

    public void setPayStatus(String payStatus) {
        this.payStatus = payStatus;
    }

    public String getPay_pwd() {
        return pay_pwd;
    }

    public void setPay_pwd(String pay_pwd) {
        this.pay_pwd = pay_pwd;
    }

    public String getPayway() {
        return payway;
    }

    public void setPayway(String payway) {
        this.payway = payway;
    }

    public String getPhone() {
        return phone;
    }

    public void setPhone(String phone) {
        this.phone = phone;
    }

    public String getPrepay_id() {
        return prepay_id;
    }

    public void setPrepay_id(String prepay_id) {
        this.prepay_id = prepay_id;
    }

    public String getProfit() {
        return profit;
    }

    public void setProfit(String profit) {
        this.profit = profit;
    }

    public boolean isSelected() {
        return selected;
    }

    public void setSelected(boolean selected) {
        this.selected = selected;
    }

    public String getShou_date() {
        return shou_date;
    }

    public void setShou_date(String shou_date) {
        this.shou_date = shou_date;
    }

    public String getSmark() {
        return smark;
    }

    public void setSmark(String smark) {
        this.smark = smark;
    }

    public String getSp_count() {
        return sp_count;
    }

    public void setSp_count(String sp_count) {
        this.sp_count = sp_count;
    }

    public String getSp_dealer() {
        return sp_dealer;
    }

    public void setSp_dealer(String sp_dealer) {
        this.sp_dealer = sp_dealer;
    }

    public String getSp_grounding() {
        return sp_grounding;
    }

    public void setSp_grounding(String sp_grounding) {
        this.sp_grounding = sp_grounding;
    }

    public String getSp_id() {
        return sp_id;
    }

    public void setSp_id(String sp_id) {
        this.sp_id = sp_id;
    }

    public String getSp_postage() {
        return sp_postage;
    }

    public void setSp_postage(String sp_postage) {
        this.sp_postage = sp_postage;
    }

    public String getSp_price() {
        return sp_price;
    }

    public void setSp_price(String sp_price) {
        this.sp_price = sp_price;
    }

    public String getSp_simg() {
        return sp_simg;
    }

    public void setSp_simg(String sp_simg) {
        this.sp_simg = sp_simg;
    }

    public String getSp_title() {
        return sp_title;
    }

    public void setSp_title(String sp_title) {
        this.sp_title = sp_title;
    }

    public String getSp_total() {
        return sp_total;
    }

    public void setSp_total(String sp_total) {
        this.sp_total = sp_total;
    }

    public String getTmark() {
        return tmark;
    }

    public void setTmark(String tmark) {
        this.tmark = tmark;
    }

    public String getTwuliu() {
        return twuliu;
    }

    public void setTwuliu(String twuliu) {
        this.twuliu = twuliu;
    }

    public String getTwuliuid() {
        return twuliuid;
    }

    public void setTwuliuid(String twuliuid) {
        this.twuliuid = twuliuid;
    }

    public String getU_account() {
        return u_account;
    }

    public void setU_account(String u_account) {
        this.u_account = u_account;
    }

    public String getU_delete() {
        return u_delete;
    }

    public void setU_delete(String u_delete) {
        this.u_delete = u_delete;
    }

    public String getU_id() {
        return u_id;
    }

    public void setU_id(String u_id) {
        this.u_id = u_id;
    }

    public String getU_phone() {
        return u_phone;
    }

    public void setU_phone(String u_phone) {
        this.u_phone = u_phone;
    }

    public String getUadm() {
        return uadm;
    }

    public void setUadm(String uadm) {
        this.uadm = uadm;
    }

    public String getUadmname() {
        return uadmname;
    }

    public void setUadmname(String uadmname) {
        this.uadmname = uadmname;
    }

    public String getUdate() {
        return udate;
    }

    public void setUdate(String udate) {
        this.udate = udate;
    }

    public String getVir() {
        return vir;
    }

    public void setVir(String vir) {
        this.vir = vir;
    }

    public String getVirid() {
        return virid;
    }

    public void setVirid(String virid) {
        this.virid = virid;
    }

    public String getWuliu() {
        return wuliu;
    }

    public void setWuliu(String wuliu) {
        this.wuliu = wuliu;
    }

    public String getWuliuid() {
        return wuliuid;
    }

    public void setWuliuid(String wuliuid) {
        this.wuliuid = wuliuid;
    }

    public String getWxpay() {
        return wxpay;
    }

    public void setWxpay(String wxpay) {
        this.wxpay = wxpay;
    }

    public String getYoufei() {
        return youfei;
    }

    public void setYoufei(String youfei) {
        this.youfei = youfei;
    }

    public String getYoumo() {
        return youmo;
    }

    public void setYoumo(String youmo) {
        this.youmo = youmo;
    }

    public String getYunfei() {
        return yunfei;
    }

    public void setYunfei(String yunfei) {
        this.yunfei = yunfei;
    }

    public static Creator<MrlistBean> getCREATOR() {
        return CREATOR;
    }

    @Override
    public int describeContents() {
        return 0;
    }

    @Override
    public void writeToParcel(Parcel dest, int flags) {
        dest.writeString(this.a_id);
        dest.writeString(this.aadm);
        dest.writeString(this.aadmname);
        dest.writeString(this.actid);
        dest.writeString(this.adate);
        dest.writeString(this.addr);
        dest.writeString(this.admin_id);
        dest.writeString(this.c_id);
        dest.writeString(this.color);
        dest.writeString(this.comment_body);
        dest.writeString(this.distribution);
        dest.writeString(this.dmark);
        dest.writeString(this.enddate);
        dest.writeString(this.fa_date);
        dest.writeString(this.groupid);
        dest.writeString(this.gui);
        dest.writeString(this.haveping);
        dest.writeString(this.id);
        dest.writeInt(this.isFanli);
        dest.writeString(this.islimit);
        dest.writeString(this.isorder);
        dest.writeString(this.isscore);
        dest.writeString(this.jmark);
        dest.writeString(this.lang_id);
        dest.writeString(this.limit);
        dest.writeString(this.m_date);
        dest.writeString(this.name);
        dest.writeString(this.nowcount);
        dest.writeString(this.openid);
        dest.writeString(this.or_date);
        dest.writeString(this.or_flag);
        dest.writeString(this.order_id);
        dest.writeString(this.payStatus);
        dest.writeString(this.pay_pwd);
        dest.writeString(this.payway);
        dest.writeString(this.phone);
        dest.writeString(this.prepay_id);
        dest.writeString(this.profit);
        dest.writeByte(this.selected ? (byte) 1 : (byte) 0);
        dest.writeString(this.shou_date);
        dest.writeString(this.smark);
        dest.writeString(this.sp_count);
        dest.writeString(this.sp_dealer);
        dest.writeString(this.sp_grounding);
        dest.writeString(this.sp_id);
        dest.writeString(this.sp_postage);
        dest.writeString(this.sp_price);
        dest.writeString(this.sp_simg);
        dest.writeString(this.sp_title);
        dest.writeString(this.sp_total);
        dest.writeString(this.tmark);
        dest.writeString(this.twuliu);
        dest.writeString(this.twuliuid);
        dest.writeString(this.u_account);
        dest.writeString(this.u_delete);
        dest.writeString(this.u_id);
        dest.writeString(this.u_phone);
        dest.writeString(this.uadm);
        dest.writeString(this.uadmname);
        dest.writeString(this.udate);
        dest.writeString(this.vir);
        dest.writeString(this.virid);
        dest.writeString(this.wuliu);
        dest.writeString(this.wuliuid);
        dest.writeString(this.wxpay);
        dest.writeString(this.youfei);
        dest.writeString(this.youmo);
        dest.writeString(this.yunfei);
    }

    public MrlistBean() {
    }

    protected MrlistBean(Parcel in) {
        this.a_id = in.readString();
        this.aadm = in.readString();
        this.aadmname = in.readString();
        this.actid = in.readString();
        this.adate = in.readString();
        this.addr = in.readString();
        this.admin_id = in.readString();
        this.c_id = in.readString();
        this.color = in.readString();
        this.comment_body = in.readString();
        this.distribution = in.readString();
        this.dmark = in.readString();
        this.enddate = in.readString();
        this.fa_date = in.readString();
        this.groupid = in.readString();
        this.gui = in.readString();
        this.haveping = in.readString();
        this.id = in.readString();
        this.isFanli = in.readInt();
        this.islimit = in.readString();
        this.isorder = in.readString();
        this.isscore = in.readString();
        this.jmark = in.readString();
        this.lang_id = in.readString();
        this.limit = in.readString();
        this.m_date = in.readString();
        this.name = in.readString();
        this.nowcount = in.readString();
        this.openid = in.readString();
        this.or_date = in.readString();
        this.or_flag = in.readString();
        this.order_id = in.readString();
        this.payStatus = in.readString();
        this.pay_pwd = in.readString();
        this.payway = in.readString();
        this.phone = in.readString();
        this.prepay_id = in.readString();
        this.profit = in.readString();
        this.selected = in.readByte() != 0;
        this.shou_date = in.readString();
        this.smark = in.readString();
        this.sp_count = in.readString();
        this.sp_dealer = in.readString();
        this.sp_grounding = in.readString();
        this.sp_id = in.readString();
        this.sp_postage = in.readString();
        this.sp_price = in.readString();
        this.sp_simg = in.readString();
        this.sp_title = in.readString();
        this.sp_total = in.readString();
        this.tmark = in.readString();
        this.twuliu = in.readString();
        this.twuliuid = in.readString();
        this.u_account = in.readString();
        this.u_delete = in.readString();
        this.u_id = in.readString();
        this.u_phone = in.readString();
        this.uadm = in.readString();
        this.uadmname = in.readString();
        this.udate = in.readString();
        this.vir = in.readString();
        this.virid = in.readString();
        this.wuliu = in.readString();
        this.wuliuid = in.readString();
        this.wxpay = in.readString();
        this.youfei = in.readString();
        this.youmo = in.readString();
        this.yunfei = in.readString();
    }

    public static final Parcelable.Creator<MrlistBean> CREATOR = new Parcelable.Creator<MrlistBean>() {
        @Override
        public MrlistBean createFromParcel(Parcel source) {
            return new MrlistBean(source);
        }

        @Override
        public MrlistBean[] newArray(int size) {
            return new MrlistBean[size];
        }
    };
}
